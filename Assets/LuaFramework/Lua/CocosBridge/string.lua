...
function string.formatString(str, ...)
...
function string.SubUrl(str, startStr, endStr)
    -- body
    local startPos = string.find(str, startStr) + string.len(startStr)
    local endPos = string.find(str, endStr) - string.len(endStr)
...
function string.reps(target,reps)
    local resultStrList = {}
    string.gsub(target,'[^'..reps..']+',function ( w )
          table.insert(resultStrList,w)
     end)
     return resultStrList
end

function string.getAndroidAPI(operatingSystem)
    --Android OS 10 \\/ API-29 (QKQ1.191117.002\\/V11.0.10.0.QJBCNXM)
    --Android OS 9 \\/ API-28 (PPR1.180610.011\\/eng.compil.20200222.000205) 
    local s0 = string.reps(operatingSystem,"(")
    --[[
        {
            1 = "Android OS 10 \/ API-29",
            2 = "QKQ1.191117.002\/V11.0.10.0.QJBCNXM)"
        }
    ]]
    local s1 = string.reps(s0[1],"-") 
    --[[
        {
            1 = "Android OS 10 \/ API",
            2 = "29"
    ...
    if s1[2] and string.len(s1[2]) > 0 then
        return tonumber(s1[2])
    end
    return 0
end